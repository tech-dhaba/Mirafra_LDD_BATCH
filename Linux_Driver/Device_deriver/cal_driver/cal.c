...
static ssize_t device_write(struct file *file, const char __user *user_buffer, size_t size, loff_t *offset) {
    ...
    if (size > BUFFER_SIZE - 1) {  // Ensure buffer doesn't overflow
        return -EINVAL;
    }
    ...
    device_buffer[size] = '\0'; // Null-terminate the string after copying

    // Parse the expression: num1 num2 operator (e.g., "3 4 +")
    if (sscanf(device_buffer, "%d %d %c", &num1, &num2, &operator) != 3) {
        printk(KERN_ALERT "simple_device: Invalid input format\n");
    ...
    if (operator == '+')
   	...
   	{
        printk(KERN_ALERT "simple_device: Invalid operator\n");
        return -EINVAL;
    }

    // Store the result in device_buffer as a string
    snprintf(device_buffer, BUFFER_SIZE, "%d", res);

    printk(KERN_INFO "simple_device: Performed calculation: %d %c %d = %d\n", num1, operator, num2, res);
