...
static ssize_t device_read(struct file *file, char __user *user_buffer, size_t size, loff_t *offset) {
	...
	{
		strcpy(device_buffer,dict[key[0]-48].value);
...
static ssize_t device_write(struct file *file, const char __user *user_buffer, size_t size, loff_t *offset) {
	...
	if(index_write<10){
		...
		sscanf(device_buffer,"%s %s",k,v);
		strcpy(dict[index_write].key,k);
		strcpy(dict[index_write].value,v);
		if(strcmp(dict[index_write].key,"key")!=0){
			index_write++;
			strcpy(key,"\0");}
		else 
			strcpy(key,dict[index_write].value);
	}
/*	else
	{
		strcpy(device_buffer,"LIMIT EXCEEDED");
	}*/	
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("KGCSR");
MODULE_DESCRIPTION("Key value Device Driver");
