...
static ssize_t gpio_write(struct file *file, const char __user *buf, size_t len, loff_t *offset) {
    ...
    buffer[len] = '\0';
    int value = simple_strtol(buffer, NULL, 10);//convert to string
...
static int __init gpio_driver_init(void) {//called when insmod
    ...
    // Register device
    ret = register_chrdev(240, "gpio_driver", &gpio_fops);//forcefully givig major number 240   if u give 0 kernel will give
    if (ret < 0) {
...
static void __exit gpio_driver_exit(void) {
    unregister_chrdev(240, "gpio_driver");//unregister
    gpio_free(GPIO_PIN);
