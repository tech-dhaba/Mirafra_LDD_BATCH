...
static ssize_t device_write(struct file *file, const char __user *user_buffer, size_t size, loff_t *offset) {
    ...
    sscanf(device_buffer,"%s %s",s[index].key,s[index].val);
    printk(KERN_INFO "key at index %d=%s val at index %d= %s\n",index,s[index].key,index,s[index].val);
   if((device_buffer[0]-48)<5)
	   sprintf(device_buffer,"%s",s[device_buffer[0]-48].key);
   else
    	snprintf(device_buffer,sizeof(device_buffer),"%s","key not found");
    index++;
