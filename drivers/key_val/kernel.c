...
/*	else
	{		
		if (copy_from_user(key, user_buffer,strlen(key)+1)) 
		{
    	    return -EFAULT;
   		}
		printk(KERN_INFO "THE key  is %s",key);
		...
		for(k=0;k<count;k++)
		{
			if(key==s[k].key)
			{
				...
				snprintf(buffer, sizeof(buffer), "%s %s",s[k].key,s[k].value);
			   	if (copy_from_user(device_buffer,buffer,strlen(buffer)+1))
   		 		{
   		    		 return -EFAULT;
   		 		}
				printk(KERN_INFO "THE  buffer is %s",buffer);

			}
		}
		snprintf(buffer, sizeof(buffer), "%s","nothing to read");
		if (copy_from_user(device_buffer,buffer,strlen(buffer)+1))
   		 {
   		     return -EFAULT;
   		 }
		printk(KERN_INFO "THE  buffer...... is %s",device_buffer);
	}*/
	return size;
}
