...
static ssize_t device_write(struct file *filep, const char __user *user_buffer, size_t size, loff_t *offset) {
    ...
    if (copy_from_user(device_buffer, user_buffer, size)) {
        printk(KERN_ERR "key_value_driver: Failed to copy data from user\n");
        return -EFAULT;
    }

    device_buffer[size] = '\0';
    printk(KERN_INFO "key_value_driver: Received %zu bytes: %s\n", size, device_buffer);

    while (write_offset < size && total_pairs < 10) { // Limit to 10 pairs
        int key, value;
        int ret = sscanf(device_buffer + write_offset, "%d:%d", &key, &value);
        if (ret != 2) {
            printk(KERN_WARNING "key_value_driver: Invalid input at offset %zu\n", write_offset);
            return -EINVAL;
        }
        keys[total_pairs] = key;
        values[total_pairs] = value;
    ...
    for (size_t i = 0; i < total_pairs; i++) {
        printk(KERN_INFO "key_value_driver: Key[%zu]: %d, Value[%zu]: %d\n", i, keys[i], i, values[i]);
    }
...
static ssize_t device_read(struct file *filep, char __user *user_buffer, size_t size, loff_t *offset) {
    ...
    for (int i = 0; i < 10; i++) {
        if (keys[i] == 0 && values[i] == 0) // Skip uninitialized pairs
            continue;
    ...
    if (copy_to_user(user_buffer, temp_buffer, output_len)) {
        printk(KERN_ERR "key_value_driver: Failed to copy data to user\n");
        return -EFAULT;
    }
