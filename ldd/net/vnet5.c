...
// Define the sysfs attribute directly (without using __ATTR macro)
static struct kobj_attribute pingdriver_attr = {
    .attr = { .name = "pingdriver", .mode = 0666 },  // Set permission to 0666
    .show = pingdriver_show,
    .store = pingdriver_store,
};

static struct attribute *attrs[] = { &pingdriver_attr.attr, NULL };
static struct attribute_group pingdriver_attr_group = { .attrs = attrs };
...
static int __init mynet_init(void) {
    ...
    // Create sysfs entry
    ret = sysfs_create_group(&dev->dev.kobj, &pingdriver_attr_group);
    if (ret) {
...
// sysfs show function
static ssize_t pingdriver_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf) {
    return snprintf(buf, PING_BUFFER_SIZE, "%s", ping_buffer);
}
