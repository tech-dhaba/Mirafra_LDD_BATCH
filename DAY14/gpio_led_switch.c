...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Vajid");
MODULE_DESCRIPTION("Sysfs example for controlling LED through switch");
MODULE_VERSION("1.0");

#define LED_GPIO 522 // GPIO pin for the LED
#define SWITCH_GPIO 523 // GPIO pin for switch
// Sysfs attributes
...
static int gpio_init(void) {
    ...
    ret = gpio_request(SWITCH_GPIO, "sysfs_switch");
    if (ret) {
        pr_err("Failed to request GPIO %d\n", LED_GPIO);
        return ret;
    }

    ret = gpio_direction_output(SWITCH_GPIO, 1);
    if (ret) {
    ...
    pr_info("GPIO %d initialized as INPUT\n", LED_GPIO);
    return 0;
}
...
static void gpio_cleanup(void) {
    ...
    gpio_set_value(SWITCH_GPIO,0); //making LOW switch
    gpio_free(LED_GPIO);
    gpio_free(SWITCH_GPIO);
    pr_info("GPIO %d & %d freed\n", LED_GPIO,SWITCH_GPIO);
}
