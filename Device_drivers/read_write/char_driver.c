...
static ssize_t device_write(struct file *file, const char __user *user_buffer, size_t size, loff_t *offset) {
	...
	/*int num;
	kstrtoint(device_buffer,10,&num);
	sprintf(device_buffer,"%d",num*2);*/
	int num;
	if(kstrtoint(device_buffer,10,&num)<0)
	{
		printk(KERN_ERR "invalid numeric");
		return -EINVAL;
	}
	sprintf(device_buffer,"%d",num*num);
