...
#define GPIO_BUTTON  529  // GPIO pin for the button gpio 17
#define GPIO_LED     539  // GPIO pin for the LED gpio 27
...
static int __init gpio_driver_init(void)
{
    ...
    // Set debounce for button GPIO
    button_desc = gpio_to_desc(GPIO_BUTTON);
    if (!button_desc) {
        pr_err("Failed to get GPIO descriptor for button\n");
        goto r_gpio_free_button;
    }
    gpiod_set_debounce(button_desc, 200 * 1000); // 200ms debounce in microseconds
