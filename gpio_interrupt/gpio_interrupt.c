...
#define GPIO_BUTTON  531  // GPIO pin for the button
#define GPIO_LED     539  // GPIO pin for the LED
...
static int __init gpio_driver_init(void)
{
    ...
    if ((alloc_chrdev_region(&dev, 0, 1, "gpio_interrupt")) < 0) {
    ...
    // Create struct class
    if (IS_ERR(dev_class = class_create("gpio_class"))) {
    ...
    //gpiod_set_debounce(GPIO_BUTTON, 200); // Debounce time in ms
      struct gpio_desc *button_desc = gpio_to_desc(GPIO_BUTTON);
    if (!button_desc) {
        pr_err("Failed to get gpio_desc for GPIO_BUTTON\n");
        goto r_gpio_free_button;
    }
    gpiod_set_debounce(button_desc, 200 * 1000); // Debounce time in microseconds
